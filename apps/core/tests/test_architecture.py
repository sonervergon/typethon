...
def check_imports(directory, forbidden_imports, error_msg_template):
    ...
    for file_path in py_files:
        ...
        for forbidden_prefix in forbidden_imports:
            illegal_imports = [imp for imp in imports if imp.startswith(forbidden_prefix)]
            if illegal_imports:
                violation = {
                    'file': file_path.name,
                ...
                print(error_msg_template.format(
                    file=file_path.name, 
...
def test_api_layer_architecture():
    ...
    assert not violations, "API layer should not import directly from operations or models layers"
...
def test_service_layer_architecture():
    ...
    assert not violations, "Service layer should not import directly from models layer (use operations layer instead)"
...
def test_all_architectural_boundaries():
    ...
    layer_boundaries = {
        'api': ['operations', 'models'],  # API should not import from operations or models
